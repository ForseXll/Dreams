...
export default function Cart() {
  ...
  return (
    <AnimatePresence>
      ...
      <motion.aside
        className={cn(
          "fixed top-0 right-0 bottom-0 z-50 grid h-full w-full min-w-0 grid-rows-[auto_1fr_auto]",
          "border-l border-[var(--color-border)] bg-[var(--color-surface-elevated)]",
          "shadow-[-8px_0_30px_rgba(0,0,0,0.1)]",
          "sm:w-[min(42vw,56rem)] sm:min-w-[36rem]"
        )}
        initial={{ x: '100%' }}
        animate={{ x: app.cartOpen ? 0 : '100%' }}
        exit={{ x: '100%' }}
        transition={{ 
          type: 'spring', 
          damping: 30, 
      ...
      >
        {/* Header */}
        <header className="border-b border-[var(--color-border)] bg-[var(--color-surface)]/50 px-4 py-5 backdrop-blur-xl sm:px-6">
        ...
        {/* Cart Items */}
        <ul className="overflow-auto px-4 py-4 sm:px-6">
          <AnimatePresence mode="popLayout">
            {app.cart.length ? (
              app.cart.map((cartItem, index) => (
                <motion.div
                  key={cartItem.id}
        ...
        {/* Footer */}
        <footer className="border-t border-[var(--color-border)] bg-[var(--color-surface)]/50 px-4 py-5 backdrop-blur-xl sm:px-6">
