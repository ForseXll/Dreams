...
export function addItemToCart(input: AddItemToCartInput) {
  return apiClient.post<AddItemToCartResponse>('/api/cart', input);
}

export function updateCartItem(id: string | number, input: UpdateCartItemInput) {
  return apiClient.put<UpdateCartItemResponse>(`/api/cart/${id}`, input);
}

export function removeCartItem(id: string | number) {
  return apiClient.delete<RemoveCartItemResponse>(`/api/cart/${id}`);
}

export function clearCart() {
  return apiClient.delete<ClearCartResponse>('/api/cart');
}
